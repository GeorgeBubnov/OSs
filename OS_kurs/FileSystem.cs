    ...
    class FileSystem
    {
        ...
        public bool ChangeDir(string name)
        {
            if (name != ".")
            {
                ...
                using (FileStream fs = new FileStream(path, FileMode.Open))
                {
                    ...
                    fs.Seek(Directory + 10, SeekOrigin.Begin);
                    fs.Read(two, 0, 2);
                    ...
                    for (int i = 0; i < count; i += 2)
                    {
                        ...
                        fs.Seek(addr + 8, SeekOrigin.Begin);
                        fs.Read(one, 0, 1);
                        UInt16 temp = (UInt16)one[0];
                        fs.Seek(addr + 9, SeekOrigin.Begin);
                        fs.Read(one, 0, 1);
                        UInt16 gid = (UInt16)one[0];
                        if (!(rights[2] == 'r' && temp == UserID) && !(rights[5] == 'r' && gid == GroupID))
                            continue;
                        fs.Seek(addr + 30, SeekOrigin.Begin);
                        fs.Read(two, 0, 2);
            ...
            else
            {
                using (FileStream fs = new FileStream(path, FileMode.Open))
                {
                    ...
                    fs.Seek(temp + 22, SeekOrigin.Begin);
                    fs.Read(buffer, 0, 2);
        ...
        public void CreateDir(string name)
        {
            if (Directory != 60)
            {
                ...
                using (FileStream fs = new FileStream(path, FileMode.Open))
                {
                    ...
                    fs.Read(eight, 0, 8);
                    string rights = GetValidString(eight);
                    fs.Seek(Directory + 8, SeekOrigin.Begin);
                    fs.Read(one, 0, 1);
                    UInt16 temp = (UInt16)one[0];
                    fs.Seek(Directory + 9, SeekOrigin.Begin);
                    fs.Read(one, 0, 1);
                    UInt16 gid = (UInt16)one[0];
                    if (!(rights[3] == 'w' && temp == UserID) && !(rights[6] == 'w' && gid == GroupID))
                        return;
                }
            }
                ...
                for (int i = 0; i < count; i += 2)
                {
                    ...
                    if (temp == UserID || 0 == UserID)
                    {
                        ...
                        if (res == fullname)
                        {
                            ...
                            if (count - i > 0)
                            {
                                fs.Seek(blockOffset + 26 + i, SeekOrigin.Begin);
                                byte[] buffer = new byte[count - i];
                                fs.Read(buffer, 0, count - i);
                                fs.Seek(-2, SeekOrigin.Current);
                                fs.Write(BitConverter.GetBytes((UInt16)0), 0, 2);
                                fs.Seek(blockOffset + 24 + i, SeekOrigin.Begin);
        ...
        public bool Rename(string name, string newName)
        {
            ...
            using (FileStream fs = new FileStream(path, FileMode.Open))
            {
                ...
                for (int i = 0; i < count; i += 2)
                {
                    ...
                    fs.Seek(inode + 8, SeekOrigin.Begin);
                    fs.Read(one, 0, 1);
                    UInt16 temp = (UInt16)one[0];
                    if (temp == UserID || 0 == UserID)
                    {
                        fs.Seek(inode + 30, SeekOrigin.Begin);
                        fs.Read(two, 0, 2);
                        temp = (UInt16)BitConverter.ToInt16(two, 0);
                        fs.Seek(temp, SeekOrigin.Begin);
                        fs.Read(twenty, 0, 20);
        ...
        public string ReadDirectory()
        {
            ...
            using (FileStream fs = new FileStream(path, FileMode.Open))
            {
                ...
                for (int i = 0; i < count; i += 2)
                {
                    ...
                    fs.Read(one, 0, 1);
                    UInt16 temp = (UInt16)one[0];
                    fs.Seek(5062 + temp * 42, SeekOrigin.Begin);
                    fs.Read(twenty, 0, 20);
                    res += GetValidString(twenty) + "\t";
                    fs.Seek(addr + 10, SeekOrigin.Begin);
                    fs.Read(two, 0, 2);
                    temp = (UInt16)BitConverter.ToInt16(two, 0);
                    res += temp + "\t";
                    fs.Seek(2, SeekOrigin.Current);
        ...
        public void WriteNewFullName(UInt16 address, string name, string expansion)
        {
            using (FileStream fs = new FileStream(path, FileMode.Open))
            {
                ...
                fs.Seek(newBladdress, SeekOrigin.Begin);
                ...
                fs.Seek(newBladdress + 510, SeekOrigin.Begin);
                fs.Write(BitConverter.GetBytes((UInt16)0), 0, 2);
                fs.Seek(address + 10, SeekOrigin.Begin);
                fs.Write(BitConverter.GetBytes((UInt16)0), 0, INode.SizeInBytesSize);
        ...
        public void WriteSuperBlock()
        {
            using (FileStream fs = new FileStream(path, FileMode.Create))
            {
                ...
                for (int i = 0; i < 20; i++)
                {
                    SuperBlock.ListINode[i] = (UInt16)(110 + i * 50);
                    fs.Write(BitConverter.GetBytes(SuperBlock.ListINode[i]), 0, 2);
                }
        ...
        public void WriteDataInBlock(UInt16 address, byte[] data)
        {
            ...
            {
                byte[] buffer = new byte[2];
                fs.Seek(address + 30, SeekOrigin.Begin);
                fs.Read(buffer, 0, 2);
                UInt16 bladdress = (UInt16)BitConverter.ToInt16(buffer, 0);
                fs.Seek(Directory + 10, SeekOrigin.Begin);
                fs.Read(buffer, 0, 2);
                UInt16 count = (UInt16)BitConverter.ToInt16(buffer, 0);
                fs.Seek(bladdress + 24 + count, SeekOrigin.Begin);
                fs.Write(data, 0, data.Length);
                if (data.Length < 488)
                {
                    fs.Seek(bladdress + 510, SeekOrigin.Begin);
                    fs.Write(BitConverter.GetBytes((UInt16)0), 0, 2);
                }
                fs.Seek(address + 10, SeekOrigin.Begin);
        ...
        public bool ChangeGroup(string login, string group)
        {
            ...
            using (FileStream fs = new FileStream(path, FileMode.Open))
            {
                for (int i = 5062; i < 5480; i += 42)
                {
                    ...
                    fs.Seek(i, SeekOrigin.Begin);
                    fs.Read(buffer, 0, 20);
                    if (GetValidString(buffer) == login)
                    {
